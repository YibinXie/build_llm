...
class DataLoaderLite:
    def __init__(self, B, T, process_rank, num_processes, split):
        ...
        data_root = "/root/autodl-tmp/data/fineweb_edu_10BT_tokenized"
...
total_batch_size = 524288  # 524288 2**19, ~0.5M, in number of tokens
...
checkpoint_path = os.path.join(log_dir, f"model_15000.pt")
if os.path.exists(checkpoint_path):
    checkpoint = torch.load(checkpoint_path)
    raw_model.load_state_dict(checkpoint['model'])
    # optimizer.load_state_dict(checkpoint['optimizer'])
    step = checkpoint['step']
    val_loss_accum = checkpoint['val_loss']
    if master_process:
        print(f"resuming training from step {step}, val_loss={val_loss_accum:.4f}")
...
send_wx_message("train_gpt2.py finished")
