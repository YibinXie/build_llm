...
class KPairwiseLoss(nn.Module):
    ...
    def forward(self, scores: torch.Tensor):
        ...
        total_comparisons = math.comb(C, 2)
        expectation = torch.sum(log_odds, dim=(1, 2)) / total_comparisons
        loss = -(1 / total_comparisons) * expectation.mean()
        return loss
...
class PolicyLoss(nn.Module):
    ...
    def forward(self, new_actor_log_probs: torch.Tensor, old_actor_log_probs: torch.Tensor, advantage: torch.Tensor):
...
class ValueLoss(nn.Module):
    ...
    def forward(self, values: torch.Tensor, reward: torch.Tensor, old_values: torch.Tensor):
        """
